...
const housingTypeSelectElement =
  formMapFiltersElement.querySelector("#housing-type");
const housingPriceSelectElement =
  formMapFiltersElement.querySelector("#housing-price");
const housingRoomsSelectElement =
  formMapFiltersElement.querySelector("#housing-rooms");
const housingGuestsSelectElement =
  formMapFiltersElement.querySelector("#housing-guests");
const housingFeaturesElement =
  formMapFiltersElement.querySelector("#housing-features");
const mapFeaturesCheckboxList =
  housingFeaturesElement.querySelectorAll("input");

const getPopupContentElement = (marker, selector) =>
  marker.getPopup().getContent().querySelector(selector);

const getFeaturesFromNotice = (marker) => {
  const noticeFeatureList = Array.from(
    getPopupContentElement(marker, ".popup__features").querySelectorAll(
      ".popup__feature"
    )
  ).map((element) => {
    const features = [];
...
const filterHousingFeatures = (marker) => {
  ...
  const isIncludesFeatures = filteredFeatures.every((feature) =>
    noticeFeatureList.includes(feature)
  );
...
formMapFiltersElement.addEventListener(
  "change",
  onCallbackReturn(debounce(onFilter))
);

const setActiveStateFormMapFilters = () => {
  const mapFiltersElement =
    formMapFiltersElement.querySelectorAll(".map__filter");
  mapFiltersElement.forEach((mapFilter) => {
    mapFilter.disabled = false;
  });

  const mapFeaturesElement =
    formMapFiltersElement.querySelector("#housing-features");
